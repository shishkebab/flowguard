...
public class Firewall implements IFirewallService, IOFMessageListener,
    ...
    public void init(FloodlightModuleContext context)
        ...
        deviceSource = context.getServiceImpl(IDeviceService.class);
    ...
    public void startUp(FloodlightModuleContext context) {
        ...
        try {
            ...
            bw.write("startup time(micro seconds) : " + ( end - start )/1000 +"\n");
            bw.flush();
            fw.close();
        } catch (IOException e) {
            e.printStackTrace();
    ...
    public Command receive(IOFSwitch sw, OFMessage msg, FloodlightContext cntx) {
        ...
        case PACKET_IN:
            ...
            if (cntx != null) {
                ...
                //System.out.println("!!!this is packet_in part!!!");
    ...
    public void enableFirewall(boolean enabled){
        ...
        try {
            ...
            bw.write(" enableFirewall time(micro seconds) : " + ( end - start )/1000 +"\n");
            bw.flush();
            fw.close();
        } catch (IOException e) {
            e.printStackTrace();
    ...
    public List<Map<String, Object>> getStorageRules() {
        ...
        try {
            ...
            logger.error("failed to Faccess storage: {}", e.getMessage());
            // if the table doesn't exist, then wait to populate later via
            // setStorageSource()
        }
        return l;
    ...
    public synchronized void addRule(FirewallRule rule) {
        ...
        try {
            ...
            bw.write("addrule time(micro seconds) : " + ( end - start )/1000 +"\n");
            bw.flush();
            fw.close();
        } catch (IOException e) {
            e.printStackTrace();
    ...
    void parseRow(Map<String, Object> row, Map<String, Map<String, OFFlowMod>> entries) {
        ...
        if (!row.containsKey(STATICENTRY_COLUMN_SWITCH) || !row.containsKey(STATICENTRY_COLUMN_NAME)) {
            logger.debug(
        ...
        try {
            ...
            switchName = (String) row.get(STATICENTRY_COLUMN_SWITCH);
            ...
            for (String key : row.keySet()) {
                if (row.get(key) == null)
                    continue;
                if (key.equals(STATICENTRY_COLUMN_SWITCH) || key.equals(STATICENTRY_COLUMN_NAME)
                        || key.equals("id"))
                    continue; // already handled
                // explicitly ignore timeouts and wildcards
                if (key.equals(STATICENTRY_COLUMN_HARD_TIMEOUT) || key.equals(STATICENTRY_COLUMN_IDLE_TIMEOUT) ||
                        key.equals(STATICENTRY_COLUMN_WILDCARD))
                    continue;
                if (key.equals(STATICENTRY_COLUMN_ACTIVE)) {
                    if  (!Boolean.valueOf((String) row.get(STATICENTRY_COLUMN_ACTIVE))) {
                        logger.debug("skipping inactive entry {} for switch {}",
            ...
            if (entryName != null && switchName != null) {
                ...
                logger.warn("Skipping entry with bad data: {} :: {} ",
    ...
    protected void readTopologyConfigFromStorage() {
        ...
        for(Iterator<IResultSet> it = resultSet.iterator();it.hasNext();){
        	...
        	for (String key : row.keySet()) {
                if (row.get(key) == null)
                    continue;
                TopologyStruct src = new TopologyStruct();
                src.dpid = row.get(LINK_SRC_SWITCH).toString();
                src.port = row.get(LINK_SRC_PORT).toString();
                TopologyStruct dst = new TopologyStruct();
                dst.dpid = row.get(LINK_DST_SWITCH).toString();
                dst.port = row.get(LINK_DST_PORT).toString();
                TopologyStorage.put(src, dst);
                break;
        	}
        }
    }
        ...
        for (String dpid : entriesToAdd.keySet()) {
            ...
            for(String entry : entriesToAdd.get(dpid).keySet()) {
                ...
                if (newFlowMod != null) {
                    ...
                    sg.staticEntryModified(dpid, entry, newFlowMod);
    ...
    public void rowsDeleted(String tableName, Set<Object> rowKeys) {
        ...
        for(Object obj : rowKeys) {
            if (!(obj instanceof String)) {
                logger.debug("Tried to delete non-string key {}; ignoring", obj);
                continue;
            }

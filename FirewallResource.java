...
public class FirewallResource extends ServerResource {
    ...
    public Object handleRequest() {
        ...
        if (op.equalsIgnoreCase("enable")) {
            ...
            ShiftedGraph sg = firewall.getShiftedGraph();
            if(sg.resolution_method == 0)
            	return "{\"status\" : \"success\", \"details\" : \"firewall running\"}";
            else if(sg.resolution_method == 1){
            	sg.resolution_method = 0;
            	return "{\"S1-Dependency Breaking applied!!\"}";
            }else if(sg.resolution_method == 2){
            	sg.resolution_method = 0;
            	return "{\"S2-Update Rejecting applied!!\"}";
            }else if(sg.resolution_method == 3){
            	sg.resolution_method = 0;
            	return "{\"S3-Flow Removing applied!!\"}";
            }else if(sg.resolution_method == 4){
            	sg.resolution_method = 0;
            	return "{\"S4-Packet Blocking applied!!\"}";
            }else if(sg.resolution_method == 5){
            	sg.resolution_method = 0;
            	return "{\"S1-Dependency Breaking applied!!\", \"S4-Packet Blocking applied!!\"}";
            }
